...
# wasm2native compilation options mapping
w2n_target_options_map = {
    "i386": ["--target=i386"],
    "x86_32": ["--target=i386"],
...
parser.add_argument('--wasm2native-compiler', type=str,
        default=os.environ.get("WASM2NATIVE_CMD", "wasm2native"),
        help="Path to wasm2native compiler")
parser.add_argument('--vmlib-file', type=str,
        default=os.environ.get("VMLIB_FILE", "libvmlib.a"),
        help="Path to wasm2native vmlib")
...
def test_assert_return(r, opts, form):
    ...
    if m and not n:
        ...
        r = compile_wasm_to_object(module+".wasm", module+".w2n", True, opts, r)
        try:
...
def compile_wasm_to_object(wasm_tempfile, native_tempfile, runner, opts, r, output = 'default'):
    log("Compiling '%s' to '%s'" % (wasm_tempfile, native_tempfile + ".o"))
    cmd = [opts.wasm2native_compiler]

    if test_target in w2n_target_options_map:
        cmd += w2n_target_options_map[test_target]

    if not opts.simd:
        cmd.append("--disable-simd")

    if output == 'ir':
        cmd.append("--format=llvmir-unopt")
    ...
    cmd += ["-o", native_tempfile + '.o', wasm_tempfile]
...
def run_wasm_with_repl(wasm_tempfile, native_tempfile, opts, r):
    cmd_native = [native_tempfile, "--repl"]
...
def test_assert_with_exception(form, wast_tempfile, wasm_tempfile, native_tempfile, opts, r, loadable = True):
    ...
    r = compile_wasm_to_object(wasm_tempfile, native_tempfile, True, opts, r)
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        for form in forms:
            ...
            if ";;" == form[0:2]:
                log(form)
            elif skip_test(form, C_SKIP_TESTS):
                log("Skipping test: %s" % form[0:60])
            elif re.match("^\(assert_trap\s+\(module", form):
                test_assert_with_exception(form, wast_tempfile, wasm_tempfile, native_tempfile, opts, r)
                ...
                test_assert_with_exception(form, wast_tempfile, wasm_tempfile, native_tempfile, opts, r, False)
