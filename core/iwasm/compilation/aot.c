...
aot_create_comp_data(WASMModule *module, aot_comp_option_t option)
{
    ...
    if (option->no_sandbox_mode) {
        if (module->import_memory_count > 0) {
            aot_set_last_error("cannot import wasm memory in no-sandbox mode.");
            return NULL;
        }

        for (i = 0; i < module->import_function_count; i++) {
            if (!strncmp(module->import_functions[i].u.function.module_name,
                         "wasi_", 5)) {
                aot_set_last_error(
                    "cannot import wasi function in no-sandbox mode.");
                return NULL;
            }
        }

        if (module->possible_memory_grow) {
            aot_set_last_error("cannot grow memory in no-sandbox mode.");
            return NULL;
        }
        if (module->memory_count > 0) {
            bh_assert(module->memories[0].init_page_count
    ...
    comp_data->data_seg_count = module->data_seg_count;
    comp_data->data_segments = module->data_segments;
    ...
    comp_data->code_reloc_count = module->code_reloc_count;
    comp_data->code_relocs = module->code_relocs;
    comp_data->data_reloc_count = module->data_reloc_count;
    comp_data->data_relocs = module->data_relocs;
    comp_data->symbol_count = module->symbol_count;
    comp_data->symbols = module->symbols;
