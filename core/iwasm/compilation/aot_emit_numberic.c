                ...
                if (arith_op == INT_DIV_S) {
                    if (!comp_ctx->no_sandbox_mode) {
                        LLVM_BUILD_ICMP(LLVMIntEQ, left,
                                        is_i32 ? I32_MIN : I64_MIN, overflow,
                                        "overflow");
                        ADD_BASIC_BLOCK(check_overflow_succ,
                                        "check_overflow_success");

                        /* Throw conditional exception if overflow */
                        if (!(aot_emit_exception(
                                comp_ctx, func_ctx, EXCE_INTEGER_OVERFLOW, true,
...
is_targeting_soft_float(AOTCompContext *comp_ctx, bool is_f32)
{
     ...
     * so user must specify '--cpu-features=+soft-float' to wasm2native if
     * the target doesn't have or enable FPU on arm, x86 or mips. */
    if (is_target_arm(comp_ctx) || is_target_x86(comp_ctx)
         ...
         * so user must specify '--cpu-features=-fp' to wasm2native if the
         * target doesn't have or enable Floating-Point Coprocessor Option on
         * xtensa.
         */
