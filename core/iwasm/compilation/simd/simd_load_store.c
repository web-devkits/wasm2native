...
simd_load(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx, uint32 align,
          uint64 offset, uint32 data_length, LLVMTypeRef ptr_type,
          LLVMTypeRef data_type, WASMRelocation *relocation)
{
    LLVMValueRef maddr, data;

    if (!(maddr = aot_check_memory_overflow(comp_ctx, func_ctx, offset,
                                            data_length, relocation))) {
...
{
    LLVMTypeRef v128_ptr_type = V128_PTR_TYPE;
    LLVMValueRef result;

    if (!(result = simd_load(comp_ctx, func_ctx, align, offset, 16,
                             v128_ptr_type, V128_TYPE, relocation))) {
        return false;
    }
...
aot_compile_simd_load_extend(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx,
                             uint8 opcode, uint32 align, uint64 offset,
                             WASMRelocation *relocation)
{
    ...
    if (!(sub_vector =
              simd_load(comp_ctx, func_ctx, align, offset, 8,
                        sub_vector_ptr_type, sub_vector_type, relocation))) {
        return false;
    }
                            ...
                            WASMRelocation *relocation)
{
    ...
    if (!(element = simd_load(comp_ctx, func_ctx, align, offset,
                              ...
                              element_data_types[opcode_index], relocation))) {
        return false;
    }
...
aot_compile_simd_load_lane(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx,
                           uint8 opcode, uint32 align, uint64 offset,
                           uint8 lane_id, WASMRelocation *relocation)
{
...
simd_store(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx, uint32 align,
           uint64 offset, uint32 data_length, LLVMValueRef value,
           LLVMTypeRef value_ptr_type, WASMRelocation *relocation)
{
                            ...
                            uint8 lane_id, WASMRelocation *relocation)
{
    ...
    return simd_store(comp_ctx, func_ctx, align, offset,
                      ...
                      element_ptr_types[opcode_index], relocation);
}
