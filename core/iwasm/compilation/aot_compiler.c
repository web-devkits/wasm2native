...
static WASMRelocation *
find_relocation(const AOTCompData *comp_data, WASMRelocation *curr_relocation,
                WASMRelocation *last_relocation, uint32 code_offset)
{
    if (!curr_relocation || curr_relocation > last_relocation)
        return NULL;

    do {
        if (curr_relocation->offset == code_offset)
            return curr_relocation;
        else if (curr_relocation->offset < code_offset) {
            LOG_WARNING("warning: ignored relocation (offset = 0x%x)\n",
                        curr_relocation->offset);
            curr_relocation++;
        }
        else
            return NULL;
    } while (curr_relocation <= last_relocation);
...
aot_compile_func(AOTCompContext *comp_ctx, uint32 func_index)
{
    ...
    WASMRelocation *curr_relocation = NULL;
    WASMRelocation *last_relocation = NULL;
    WASMRelocation *relocation = NULL;
    const uint8 *code_section_body = comp_data->wasm_module->code_section_body;
    ...
    /* Ignore relocations that don't belong to current function */
    for (i = 0; i < comp_data->code_reloc_count; i++) {
        if (comp_data->code_relocs[i].offset
            >= (uint32)(uintptr_t)(frame_ip - code_section_body)) {
            curr_relocation = &comp_data->code_relocs[i];
            last_relocation =
                &comp_data->code_relocs[comp_data->code_reloc_count - 1];
            break;
        }
    }
    ...
    while (frame_ip < frame_ip_end) {
        ...
        switch (opcode) {
            ...
            case WASM_OP_CALL:
                relocation = find_relocation(
                    comp_data, curr_relocation, last_relocation,
                    (uint32)(uintptr_t)(frame_ip - code_section_body));
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op call, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
            ...
            case WASM_OP_GET_GLOBAL_64:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE(
                        "op global.get, find relocation (offset = 0x%x)",
                        relocation->offset);
                }
            ...
            case WASM_OP_SET_GLOBAL_AUX_STACK:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE(
                        "op global.set, find relocation (offset = 0x%x)",
                        relocation->offset);
                }
                read_leb_uint32(frame_ip, frame_ip_end, global_idx);
            ...
            op_i64_load:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op i64.load, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            case WASM_OP_F32_LOAD:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op f32.load, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            case WASM_OP_F64_LOAD:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op f64.load, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            op_i32_store:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op i32.store, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
                if (!aot_compile_op_i32_store(comp_ctx, func_ctx, align, offset,
                                              bytes, false, relocation))
                    return false;
                break;
            ...
            op_i64_store:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op i64.store, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            case WASM_OP_F32_STORE:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op f32.store, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            case WASM_OP_F64_STORE:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op f64.store, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                read_leb_mem_offset(frame_ip, frame_ip_end, offset);
            ...
            case WASM_OP_I64_CONST:
                ...
                if (relocation) {
                    curr_relocation = relocation + 1;
                    LOG_VERBOSE("op i64.const, find relocation (offset = 0x%x)",
                                relocation->offset);
                }
                ...
                switch (opcode) {
                    ...
                    case SIMD_v128_load:
                    {
                        ...
                        if (relocation) {
                            curr_relocation = relocation + 1;
                            LOG_VERBOSE(
                                "op v128.load, find relocation (offset = 0x%x)",
                                relocation->offset);
                        }
                        read_leb_mem_offset(frame_ip, frame_ip_end, offset);
                        if (!aot_compile_simd_v128_load(
                                comp_ctx, func_ctx, align, offset, relocation))
                            return false;
                        break;
                    }
                    ...
                    case SIMD_v128_store:
                    {
                        ...
                        if (relocation) {
                            curr_relocation = relocation + 1;
                            LOG_VERBOSE("op v128.store, find relocation "
                                        "(offset = 0x%x)",
                                        relocation->offset);
                        }
                        read_leb_mem_offset(frame_ip, frame_ip_end, offset);
                        if (!aot_compile_simd_v128_store(
                                comp_ctx, func_ctx, align, offset, relocation))
                            return false;
                        break;
                    }
                    ...
                    case SIMD_v128_store64_lane:
                    {
                        ...
                        read_leb_mem_offset(frame_ip, frame_ip_end, offset);
                        if (!aot_compile_simd_store_lane(
                                comp_ctx, func_ctx, opcode, align, offset,
                                *frame_ip++, relocation))
                            return false;
                        break;
...
aot_compile_wasm(AOTCompContext *comp_ctx)
{
    ...
    /* Apply llvm passes before generating machine code */
